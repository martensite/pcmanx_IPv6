...
#endif
	{
		//if( m_InAddr.s6_addr != INADDR_NONE || inet_aton(address.c_str(), &m_InAddr) )
...
//  and be executed one by one.  So no mutex lock is needed anymore.
	//if( ! inet_pton( AF_INET6, data->m_Address.c_str(), &addr) || ! inet_aton(data->m_Address.c_str(), &addr4) )
	//{
